<script lang="ts">
    ...
    const feedingData = (events: FeedingEvent[]): LineData => {
        const transformedData = events.map(e => {
            ...
            const second = (e.startedSecond && e.stoppedSecond ? e.stoppedSecond.getTime() - e.startedSecond.getTime() : 0) / 60 / 1000;
    ...
    $: feedingEvent = feedingData($allEvents?.filter((event: BabyEvent) => event.eventType === EventType.Feeding));
